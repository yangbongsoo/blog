...
at home in such a clss should instead be put in the interface itself. Note, however that it may still be necessary
...
정적 팩토리 메서드가 반환하는 객체의 클래스는 정적 팩토리 메서드가 정의된 클래스의 코드가 작성되는 순간에 존재하지 않아도 무방하다.
...
service interface to clients than the one furnished by providers. This is the Bridge pattern(2rd 번역에서는 어댑터패턴).
Dependency injection frameworks can be viewed as powerful service providers. Since Java 6, the platform includes a
general-purpose service provider framework, java.util.ServiceLoader, so you needn't, and generally shouldn't, write
your own. JDBC doesn't use ServiceLoader, as the former predates the latter.
