...
자바8부터 인터페이스에 static 메서드를 추가할 수 있지만, 인터페이스의 모든 static 멤버들은 public이다(Java 9는 private static 메서드를 허용).
또한 인터페이스에 팩토리 메서드를 추가하면 인터페이스가 생성되는 구현 클래스를 의존하게 된다. 개인적으로는 그 방식이 좋아보이지 않는다.
