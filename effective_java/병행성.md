...
이 실행자에 Runnable을 넘겨 실행시키려면 다음과 같이 한다. `executor.execute(runnable);` 실행자가 스스로 자연스럽게 종료되도록 하려면 다음과 같이 하면 된다(이 코드를 빼먹으면 VM은 종료되지 않는다). `executor.shutdown();`
