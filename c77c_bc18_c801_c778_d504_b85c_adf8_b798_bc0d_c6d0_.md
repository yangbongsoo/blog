...
**문자열은 값 자료형(value type)을 대신하기에는 부족하다.** 데이터가 파일이나 네트워크나 키보드를 통해서 들어올 때는 보통 문자열 형태다. 그러니 그대로 두려는 경향이 있다. 하지만 데이터가 텍스트 형태일 때나 그렇게 하는 것이 ㅈ호다. 숫자라면 int, float, BigInteger 같은 수 자료형으로 변환해야 한다. 적당한 자료형이 없다면 새로 만들어야 한다.
**문자열은 enum 자료형을 대신하기에는 부족하다.**
**문자열은 혼합 자료형을 대신하기엔 부족하다.** `String compundKey = className + “#” + i.next();`
