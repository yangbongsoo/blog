...
(보통 하나의 페이지에 다수의 레코드가 일반적임)

![](diskdata.PNG)

S6 레코드를 삽입하고 S2 삭제하고 C6를 삽입하고(삭제된 S2자리에) S4를 삭제한 상황

디스크 관리자가 여러 페이지 세트들을 식별하기 위한 방법은 0번 페이지를 디스크 디렉터리로 사용한다. 

![](diskdirectory.PNG)

각 페이지 세트의 첫 번째 페이지의 주소만 알면 그 나머지 페이지들은 페이지 헤드에 있는 다음 페이지 포인터를 통해서 쉽게 식별할 수 있다. 
