...
##4. 날짜
```
long plusDay = deleteSchedule.getExecutionDate().getTime() + TimeUnit.DAYS.toMillis(extendDays);

deleteSchedule.setRegisterDate(Date.from(Instant.now()));
deleteSchedule.setExecutionDate(new Date(plugDay));
```