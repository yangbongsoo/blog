...
만약 specific configuration을 로드하고 싶으면 `@SpringBootTest`의 `classes`속성을 사용하면 된다. **`classes`속성을 생략하면 inner-classes에서 @Configuration을 제일 먼저 로드하려 시도하고, 없다면 @SpringBootApplication class를 찾는다.**



todo
@IntegrationTest 해주면 뭐가 좋은지 확인 
@webAppConfiguration 이랑 같이 쓰니까 안된던데 이유 (추가적으로 @WebIntegrationTest 안됌)
그리고 그냥 spring에서는 어떻게 통합테스트하는지 확인
