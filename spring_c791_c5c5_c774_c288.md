...
```
`@ContextConfiguration`은 통합테스트를 위해 어떻게 ApplicationContext를 로드하고 설정할지 결정하는데 사용되는 클래스 레벨 메타데이터(xml 파일 or javaConfig 파일)를 정의한다. 

다시 말해, `@ContextConfiguration`은 context를 로드하는데 사용되는 annotated class(@Configuration 클래스들을 의미)나 application context resource locations(classpath에 위치한 XML 설정 파일)들을 선언한다.

`@ContextConfiguration`은 ContextLoader 전략을 사용할 수 있다. 하지만 일반적으로 로더를 직접 명시할 필요는 없다. default loader가 initializers 뿐만 아니라 resource locations 또는 annotated classes를 지원하기 때문이다. 

---

하지만 Spring Boot에서 `@ContextConfiguration(classes = Application.class)`만 설정했더니 
```
...
...
```
위와 같은 에러뿐만 아니라 기본적인 Autowired설정도 안되고, BeanCreationException도 발생했다.
...
```
cf) 
방법3에서 ConfigFileApplicationContextInitializer는 Spring Boot application.properties파일을 로드해 테스트 코드에 적용한다. `@SpringApplicationConfiguration`가 제공하는 full feature들이 필요 없을 때 사용된다.
