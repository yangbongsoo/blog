...
```
cf) `@SpringBootTest`에서 classes 속성을 생략하면 inner-classes에서 `@Configuration`을 제일 먼저 로드하려 시도하고, 없으면 `@SpringBootApplication` class를 찾는다.



todo
@IntegrationTest 해주면 뭐가 좋은지 확인 
@WebIntegrationTest 안됌
