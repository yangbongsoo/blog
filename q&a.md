...
성능데이터의 경우 , 데이터의 양이 폭증할 가능성이 있고, 부분적으로 분실해도 큰 문제가 없기 때문에 UDP를 사용한다. API 메타데이터나, SQL정보와 같이 중요하고, 데이터양이 일정한 경우는 TCP로 보내고 있다.그리고 4번에서 native code로 변환하고 실행이 된다. 
...
또 톰캣을 통해 성능 수집을 하는 것을 Java구간의 프로파일링이라고 하는 건가요? 답) Yes
