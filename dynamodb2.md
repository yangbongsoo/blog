...
secondary index안의 데이터는 테이블에서 인덱스로 project되거나 복사된 속성들로 구성되어 있다. 모든 secondary index는 DynamoDB에 의해 자동으로 유지된다. 테이블의 아이템을 추가하거나 수정, 삭제할 때 테이블의 모든 인덱스들도 이러한 변화를 반영하기위해 업데이트된다. 