...
```
이 예에서 DynamoDB client에 updateItem()을 보내기 위해 UpdateItemRequest를 작성했다. UpdateItemRequest objcet는 update할 속성 이름을 명시한 addAttributeUpdatesEntry(속성에 대한 새로운 값, 속성에서 수행하기 위한 액션)를 호출한다. 또한 operation의 conditional block인 addExpectedEntry를 호출한다. 
