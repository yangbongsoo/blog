...
위의 코드와 같이 each는 현재 원소를 저장하는 명확하고 단순한 지역 변수 이름이다. 어떤 원소인지 궁금한 경우 간ㅌ단히 for 문을 살펴보면 된다.
...
더블 디스패치를 사용하면 유연성을 조금 앓게 되며, 코드 중복이 발생한다. 우선 첫째 선택 메시지의 수신자 타입 이름이 둘째 선택 메시지의 수신자 메서드에 들어간다.
...
유튭 영상 :  https://www.youtube.com/watch?v=s-tXAHub6vg
...
때로는 대칭성과 같은 "미학적" 만족만을 위해 새로운 메서드를 만드는 것이 의미 없게 느껴질 대도 있다. 하지만 미학은 생각보다 중요한 것이다. 미학은 엄격한 선형적 논리 사고보다 더 많은 두뇌 활동을 요구한다.
