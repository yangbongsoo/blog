...
중요한 점은 서비스(Sokit)에 대해서 해당 사용자가 페이스북 사용자임을 인증해고 서비스(Sokit)는 사용자의 비밀번호를 받지 않는다. 대신 페이스북이 사용자를 인증하고 서비스(Sokit)에 알려주는 방식이다. 즉, 파트너 서비스에는 페이스북 사용자의 비밀번호가 노출되지 않는다.
